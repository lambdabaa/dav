...
debug = debug('dav:contacts');

/**
 * @param {dav.Account} account to fetch address books for.
 */
export async function listAddressBooks(account, options) {
  ...
  return Promise.all(addressBooks.map(async function(addressBook) {
 ...
 *   (dav.Sandbox) sandbox - optional request sandbox.
 */
export async function listVCards(addressBook, options) {
 ...
 *   (dav.Transport) xhr - request sender.
 */
export async function syncCarddavAccount(account, options={}) {
  ...
  addressBooks
    .filter(function(addressBook) {
      ...
      return account.addressBooks.every(
        prev => prev.url.indexOf(addressBook.url) === -1
      );
    })
  ...
  await Promise.all(
    account.addressBooks.map(async function(addressBook, index) {
      try {
...
async function basicSync(addressBook, options) {
...
async function webdavSync(addressBook, options) {
