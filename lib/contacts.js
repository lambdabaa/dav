...
var accounts = require('./accounts'),
    debug = require('debug')('dav:contacts'),
    model = require('./model'),
...
function basicSync(addressBook, options) {
  return new Promise(function(resolve, reject) {
    ...
    .then(function(responses) {
      ...
      if (!response) {
        reject(
          new Error('Could not find address book on remote. Was it deleted?')
        );
      }
 ...
 *   (dav.Transport) xhr - request sender.
 */
function sync(addressBook, options) {
  var syncMethod;
...
exports.syncAccount = function(account, options) {
  ...
  return accounts.fetchAddressBooks(account, options)
  .then(function(addressBooks) {
    ...
    return Promise.all(
      account.addressBooks.map(function(addressBook, index) {
        return sync(addressBook, options)
