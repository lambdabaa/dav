...
export async function createAccount(options) {
  ...
  if (options.accountType === 'caldav') {
    account.calendars = await calendars.listCalendars(account, options);
  } else if (options.accountType === 'carddav') {
    account.addressBooks = await contacts.listAddressBooks(account, options);
  }
